...
export function useLogin() {
  const { mutate: loginMutate, error: loginError, ...rest } = useMutation({ mutationFn: login });
...
export function useLogout() {
  const { mutate: logoutMutate, error: logoutError, ...rest } = useMutation({ mutationFn: logout });
...
export function useVerify() {
  const { mutate: verifyMutate, error: verifyError, ...rest } = useMutation({ mutationFn: verify });
